                  ...
                  <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                    @forelse($transactions as $transaction)
                      <tr class="text-gray-700 dark:text-gray-400">
                        ...
                        <td class="px-4 py-2 text-sm"> <span class="font-semibold uppercase">{{ $transaction->particulars }}</span> {{ $transaction->getStaff ? 'to '.$transaction->getStaff->name : '' }} {{ $transaction->getCategory ? $transaction->getCategory->name : '' }} {{ $transaction->reference ? 'Reference: '.$transaction->reference : null  }} </td>
