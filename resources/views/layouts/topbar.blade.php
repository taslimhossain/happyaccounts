<header class="z-10 py-4 bg-white shadow-md dark:bg-gray-800">
    <div class="container flex items-center justify-between h-full px-6 mx-auto text-indigo-600 dark:text-indigo-300">
      ...
      <ul class="flex items-center flex-shrink-0 space-x-6">
        ...
        <li class="flex">
          <button class="rounded-md focus:outline-none focus:shadow-outline-indigo" @click="toggleTheme" aria-label="Toggle color mode" >
            <template x-if="!dark">
              <svg
                class="w-5 h-5"
                aria-hidden="true"
        ...
        <!-- Profile menu -->
        <li class="relative" x-data="{ isProfileMenuOpen: false }" @click.outside="isProfileMenuOpen = false" @close.stop="isProfileMenuOpen = false">
          <button @click="toggleProfileMenu"
            class="flex  items-center gap-4 align-middle rounded-full focus:shadow-outline-indigo focus:outline-none"
            ...
            <img
              ...
              aria-hidden="true"
            />
            <span class="font-bold text-heading cursor-pointer">@if(Auth::check()) {{ Auth::user()->name }} @endif</span>
          </button>
